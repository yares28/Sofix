...
- Docker must never receive Neon URLs.
...
| 2.4 | ✅ **Lock files and Python version:** `uv pip compile` lock; `.python-version` + `requires-python`; one version in venv and Docker | `backend/` | Reproducible install; versions match |
...
| 2.8 | ⬜ **Docker hardening:** `127.0.0.1:` port binds, non-root users, production builds (Next standalone, no `--reload`), `postgres:18`, `${POSTGRES_PASSWORD:?}` | `Dockerfile`s, compose | `docker compose up` serves prod builds on localhost only |
| 2.9 | ⬜ **API and web hardening:** docs/OpenAPI disabled outside dev; typed `GridMeta`; exception handler → 503 envelope; Next security headers (CSP allowing `crests.football-data.org`, `nosniff`, `Referrer-Policy`), `poweredByHeader: false`; server-only `API_BASE_URL` | `main.py`, `schemas.py`, `next.config.mjs`, `page.tsx` | Headers present; suspended DB → 503 JSON |
...
| 3.6 | ⬜ **Tests never touch Neon:** `tests/conftest.py` sets `POSTGRES_URL=sqlite://` before importing `app`, plus a guard | `tests/conftest.py` | Guard test fails if the URL isn't SQLite |
...
| 3.10 | ⬜ **Logging:** replace `print` with `logging` (JSON-ish lines); `fileConfig(..., disable_existing_loggers=False)`; pass migration URL via `config.attributes` (URL-encoded passwords) | `app/*`, `migrations/env.py`, `migrate.py` | Logs survive the migration step |

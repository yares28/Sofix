...
def resolve_team(db: Session, payload_team: dict, cache: dict | None = None):
    ...
    source_id = str(payload_team["id"])
    source_name = payload_team["name"]
    if cache is not None and source_id in cache:
        return cache[source_id]
    info = require_code(payload_team.get("tla"), source_name)
    sm = (
        db.query(SourceEntityMap).filter_by(entity_type="team", source="football-data.org", source_id=source_id).first()
    )
...
def upsert_fixture(db, comp, payload, teams: dict | None = None, existing: dict | None = None):
    ...
    if not fx:
        ...
        fx.matchday = payload.get("matchday")
        fx.status = status
        fx.home_goals = full.get("home")
        fx.away_goals = full.get("away")

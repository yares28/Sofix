...
from app.sources.open_meteo import hourly_forecast,at_kickoff
        ...
        try:
            for stadium_id,stadium_fixtures in by_stadium.items():
                ...
                for fx in stadium_fixtures:
                    ...
                    if row:
                        db.query(WeatherSnapshot).filter(WeatherSnapshot.fixture_id==fx.id).delete(synchronize_session=False)
                        db.add(WeatherSnapshot(fixture_id=fx.id,**row)); written+=1
        finally:
            if client is None: await http.aclose()
        db.commit(); print("weather rows",written,"for",len(by_stadium),"stadiums")
    finally:
        db.close()

...
def at_kickoff(data:dict,kickoff_utc:datetime,now:datetime|None=None)->dict|None:
    ...
    hourly=data["hourly"]
    times=[datetime.fromisoformat(x).replace(tzinfo=timezone.utc) for x in hourly["time"]]
    ...
    return {
      "snapshot_ts":now,"available_at":now,"forecast_lead_hours":(kickoff_utc-now).total_seconds()/3600,

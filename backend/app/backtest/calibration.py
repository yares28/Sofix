...
def with_difficulty(team_rows: pd.DataFrame) -> pd.DataFrame:
    scores = [difficulty_score(w, d, l) for w, d, l in team_rows[["p_win", "p_draw", "p_loss"]].itertuples(index=False)]
...
def propose_thresholds(scores: pd.Series, shares: Sequence[float] = (0.15, 0.20, 0.30, 0.20, 0.15)) -> tuple[float, ...]:
...
def band_table(team_rows: pd.DataFrame, thresholds: Sequence[float]) -> pd.DataFrame:
    ...
    table = labelled.groupby("label", observed=False).agg(
...
def clean_sheet_calibration(team_rows: pd.DataFrame, bins: int = 5) -> pd.DataFrame:
    ...
    return rows.groupby(buckets, observed=True).agg(
        fixtures=("p_cs", "size"),
        predicted=("p_cs", "mean"),
